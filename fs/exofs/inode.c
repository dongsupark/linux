...
static int pcol_try_alloc(struct page_collect *pcol)
{
	...
	if (!pcol->ios) { /* First time allocate io_state */
		if (exofs_get_io_state(pcol->sbi, &pcol->ios))
			return -ENOMEM;
	}
...
/*	u64 resid;*/
	u64 good_bytes;
	u64 length = 0;
	int ret = exofs_check_io(pcol->ios);
...
static int read_exec(struct page_collect *pcol, bool is_sync)
{
	...
	*pcol_copy = *pcol;
	...
	EXOFS_DBGMSG("read_exec obj=0x%llx start=0x%llx length=0x%lx\n",
		  ios->obj.id, _LLU(i_start), pcol->length);
		    ...
		    struct exofs_fcb *inode, uint64_t *sanity)
{
	...
	struct osd_obj_id obj = {sbi->s_pid, exofs_oi_objno(oi)};
	int ret;
...
static int exofs_update_inode(struct inode *inode, int do_sync)
{
	...
	ret = exofs_get_io_state(sbi, &ios);
	if (unlikely(ret)) {
		EXOFS_ERR("exofs_update_inode: exofs_get_io_state failed.\n");
		ret = -ENOMEM;
		goto free_args;
	}
...
void exofs_delete_inode(struct inode *inode)
{
	...
	ret = exofs_get_io_state(sbi, &ios);
	if (unlikely(ret)) {
		EXOFS_ERR("exofs_delete_inode: exofs_get_io_state failed\n");
		return;
	}
	...
	ios->private = sbi;
	ios->cred = oi->i_cred;
	ret = exofs_sbi_remove(sbi, ios);
	if (ret) {
		EXOFS_ERR(
		       "ERROR: @exofs_delete_inode exofs_sbi_remove failed\n");
		exofs_put_io_state(ios);
		return;
	}
	atomic_inc(&sbi->s_curr_pending);
